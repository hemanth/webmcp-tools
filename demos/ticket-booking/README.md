...
navigator.modelContext.registerTool({
  ...
  execute: ({ movie_id, time }) => {
    ...
    return {
      status: 'success',
      message: `Selected showtime ${time} for "${movie.title}" movie.`,
    };
  },
});
```
...
- **Select Showtime**: Choose a specific movie and its available showtime to automatically navigate to the checkout section.
